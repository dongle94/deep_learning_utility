...
def main():
    ...
    if gpus:
        ...
        tf.config.experimental.set_memory_growth(gpus[0], allow_growth)
        if memory_limits and memory_limits != 0:
            tf.config.experimental.set_virtual_device_configuration(
                gpus[0],
                [tf.config.experimental.VirtualDeviceConfiguration(memory_limits)]
            )

    detection_graph = tf1.Graph()
    tf_config = tf1.ConfigProto()
    sess = tf1.Session(config=tf_config, graph=detection_graph)
    print(f"[{os.getpid()}]========== [GPU설정] allow_growth: {allow_growth} / memory_limits: {memory_limits} ==========")
